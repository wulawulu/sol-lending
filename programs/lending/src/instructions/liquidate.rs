...
pub struct Liquidate<'info> {
    ...
    pub liquidator_borrow_token_account: InterfaceAccount<'info, TokenAccount>,
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> {
    ...
    let transfer_to_bank = TransferChecked {
        from: ctx
            .accounts
            .liquidator_borrow_token_account
            .to_account_info(),
        mint: ctx.accounts.borrowed_mint.to_account_info(),
        to: ctx.accounts.borrow_bank_token_account.to_account_info(),
